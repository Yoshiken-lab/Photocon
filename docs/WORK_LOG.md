...
2.  **Sample O 応募フォームの実装** (`/sample-o/apply`)
    ...
    #### 🔒 【修正】認証モード不具合修正
    - **不具合**: 管理画面で認証をOFFにしても、トップページのログインボタンが消えなかった。
    - **原因**:
        1.  コンポーネントがDB設定ではなく環境変数を参照していた。
        2.  修正したが、DBのキー名が `is_auth_enabled` (Toggle側) と `is_login_required` (参照側) で食い違っていた（俺の凡ミスだ！！！！）。
    - **対応**: 参照側のキーを `is_auth_enabled` に統一し、クライアントコンポーネントからDB設定を直接読み取る `getAuthMode` アクションを実装・適用した！！



    ### ⚠️ 【反省】AI実装ミスと教訓（2026-01-29追記）
    今回の「認証モード不具合」において、以下の重大なミスを犯した。これを深く反省し、再発防止のために記録する。
    1.  **影響範囲の確認不足**:
        - 管理画面で「認証OFF」機能を実装した際、フロントエンド（ヘッダーやサイドバー）が「何を見て表示を切り替えているか」の調査が不足していた。
        - 結果、フロントエンドは静的な環境変数 (`NEXT_PUBLIC_ENABLE_AUTH`) を見続けており、DB設定が無視される状態を作り出してしまった。
    2.  **キー名の不一致 (Key Mismatch)**:
        - 修正時に、設定保存側 (`components/admin/AuthModeToggle.tsx`) では `is_auth_enabled` を使用していたのに、
        - 読み取り側 (`lib/system-settings.ts`) では古い想定の `is_login_required` を指定してしまった。
        - この「単純なキーの記述ミス」により、設定値が正しくてもデフォルト値（True）が返され、修正が機能しない事態を招いた。
    
    **【教訓】**:
    - **定数管理の徹底**: DB設定のキー名などは文字列リテラルで散らばらせず、定数として一元管理すべきである。
    - **End-to-Endの動作確認**: 「コードを直した」で満足せず、実際の画面で「設定を切り替えてボタンが消えるか」を目視確認することを徹底する。
