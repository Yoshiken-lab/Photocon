...
### 20. 認証モード不整合によるログインボタン残留問題
- **発生日時**: 2026-02-05
- **現象**: 
    - 管理画面で「アカウント認証」をOFF（匿名利用）に設定しても、トップページやサイドバー等にある「ログイン」ボタンが消えない。
    - 認証OFFの状態でログインしようとしても、当然ログイン画面には行けるが、本来その導線自体が消えるべき設計だった。
- **原因**: 
    1. **参照先の不一致**: 表示側コンポーネント（`HeaderO.tsx`, `LayoutO.tsx` 等）が、DB設定値（`system_settings`）ではなく、ビルド時の環境変数（`NEXT_PUBLIC_ENABLE_AUTH`）を静的に参照していた。
    2. **キー名の不一致 (Human Error)**: 
        - 表示側をDB参照に修正する際、DB設定のキーを古い想定の `is_login_required` と記述してしまった。
        - 実際の設定保存側（`AuthModeToggle.tsx`）は `is_auth_enabled` というキーを使用しており、キー名が一致していなかったため、設定を取得できずデフォルト値（True）で動作していた。
- **解決策**:
    1. **全レイヤーでのDB設定参照化**: 環境変数への依存をやめ、Server Action `getAuthMode` 経由で常に最新のDB設定を取得するようにクライアントコンポーネントを改修。
    2. **キー名の統一**: `getAuthMode` 内の参照キーを正しい `is_auth_enabled` に修正。
- **教訓**:
    - **「設定変更機能を作ったら、全ての表示箇所が変わるか目視確認しろ！！！！」**
    - コンポーネント単位の修正で満足せず、システム全体としての挙動（E2E）を確認すること。
    - **「定数は一箇所にまとめろ！！！！」**
    - `is_auth_enabled` のような重要なキー文字列をコード内に散在させるとミスを見逃す。定数ファイル (`constants.ts` 等) で一元管理すべきだった。

### 21. revalidatePath 漏れによる公開ページのキャッシュ残留問題
- **発生日時**: 2026-02-05
- **現象**: 
    - 管理画面で入賞選択（金賞/銀賞/銅賞）を行っても、公開側の作品一覧ページ（`/sample-o/result/[id]`）に入賞バッジが反映されない。
    - ブラウザをリロードしても古い情報が表示され続け、開発サーバーを再起動（ファイル変更を検知させる）すると突然表示される。
- **原因**: 
    - `admin-ranking.ts` の `updateEntryAward` 関数で `revalidatePath('/admin/ranking')` しか呼んでいなかった。
    - Next.jsのApp Routerはルートごとにキャッシュを持つため、管理画面のキャッシュをクリアしても**公開側のページキャッシュ (`/sample-o/...`) は古いデータのまま**残っていた。
- **解決策**:
    - 入賞更新処理に以下の `revalidatePath` を追加し、公開ページのキャッシュも強制的に無効化するようにした。
    ```typescript
    revalidatePath('/sample-o', 'layout') // トップページと配下すべて
    revalidatePath('/sample-o/result', 'layout') // 結果ページすべて
    ```
- **教訓**:
    - **「DBを更新するなら、そのデータを使う全ての画面のキャッシュをクリアしろ！！！！」**
    - Server Actions で `revalidatePath` を呼ぶ際は、管理画面だけでなく、そのデータが表示される可能性のある**公開ページ（ユーザー向け画面）も忘れずに対象に含めること**。
    - Next.jsのキャッシュは賢いが、ルートの境界を越えない。意識的に複数パスを再検証する必要がある。
